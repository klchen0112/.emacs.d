...
#+begin_src emacs-lisp
  (add-hook 'emacs-startup-hook
            (lambda ()
              (message "Loaded %d Emacs in %.03fs"
                       (length features)
...
#+begin_src emacs-lisp
  ...
  (defconst my/main-fonts
    '((default        . "Pragmasevka Nerd Font")
      ...
      (bold           . "Pragmasevka Nerd Font Bold")
  ...
  (defconst my/symbol-fonts
    (if IS-MAC
      ...
      '("Symbola" "Noto Color Emoji" "Segoe UI Emoji" "Liberation Mono"
        "Noto Sans Symbols 2" "FreeSerif" "Twitter Color Emoji"))
    "Fonts used for symbols")

  ;; 辅助函数：异步检查字体是否存在
  (defun my/check-font-exists (font-name)
    "检查字体是否存在（缓存结果）"
    (or (get-text-property 0 'exists font-name)  ; 使用文本属性缓存结果
        (let ((exists (find-font (font-spec :name font-name))))
          (put-text-property 0 1 'exists exists font-name)
          exists)))
    ...
    (when (display-graphic-p)
      ...
      (dolist (face my/main-fonts)
        (let ((font-name (cdr face))
              (face-name (car face)))
          (when (my/check-font-exists font-name)
            (set-face-attribute face-name nil :font font-name))))

      ;; 2. 设置符号字体（批量设置）
      (let ((available-symbol-fonts
             (cl-remove-if-not #'my/check-font-exists my/symbol-fonts)))
        (when available-symbol-fonts
          ;; 使用 set-fontset-font 的 append 参数一次性添加多个字体
          (set-fontset-font t 'symbol (car available-symbol-fonts))
          (dolist (font (cdr available-symbol-fonts))
  ...
  (defun my/setup-cjk-fonts ()
    "设置CJK字体（可以进一步延迟或按需调用）"
    ;; 中文字体
    (when (my/check-font-exists "TsangerJinKai05")
    ...
    ;; 其他CJK字体
    (when (my/check-font-exists "Noto Sans")
      (set-fontset-font t 'cjk-misc (font-spec :family "Noto Sans")))

    ;; 西里尔字体
    (when (my/check-font-exists "Noto Serif")
      (set-fontset-font t 'cyrillic (font-spec :family "Noto Serif"))))

  ;; 按需加载CJK字体
  (defun my/setup-cjk-fonts-if-needed ()
    "在需要时加载CJK字体"
    (when (and (display-graphic-p)
               (or (memq major-mode '(org-mode markdown-mode latex-mode))
                   (string-match "\\cC" (buffer-string))))  ; 检测中文字符
      (my/setup-cjk-fonts)))
      ...
      (add-hook 'after-make-frame-functions
                (lambda (frame)
                  (with-selected-frame frame
                    (run-with-idle-timer 0.1 nil #'my/setup-fonts)))))
     ...
     ((display-graphic-p)
      ...
      (run-with-idle-timer 0.1 nil
                           (lambda ()
                             ...
                             (my/setup-fonts)
                             ;; 添加按需加载CJK字体的钩子
                             (add-hook 'find-file-hook #'my/setup-cjk-fonts-if-needed)
                             (add-hook 'after-change-major-mode-hook #'my/setup-cjk-fonts-if-needed))))
