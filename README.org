...
#+begin_src emacs-lisp
  (setq
   ...
   ;; Emacs 30+ startup optimizations
   inhibit-splash-screen t
   inhibit-startup-banner t
   inhibit-startup-help-dialog t
   inhibit-startup-tool-bar t
...
#+begin_src emacs-lisp
  ...
  ;; Emacs 30+ native-comp verbose settings
  (setq native-comp-verbose init-file-debug)
  ...
  (setup ef-themes
    ...
    (:when-loaded
      (:option
       ...
       modus-themes-disable-otehr-themes t)
...
#+begin_src emacs-lisp
  ...
  (defconst my/main-fonts
    '((default        . "M+CodeLat50 Nerd Font Propo")
      (fixed-pitch    . "M+CodeLat50 Nerd Font Propo")
      (variable-pitch . "M+CodeLat50 Nerd Font Propo")
      (bold           . "M+CodeLat50 Nerd Font Propo")
      (italic         . "M+CodeLat50 Nerd Font Propo")))

  (defconst my/symbol-fonts
    (if IS-MAC
        '("Apple Symbols" "Apple Color Emoji" "Symbola" "Noto Color Emoji"
          "Liberation Mono"
          "Segoe UI Emoji" "FreeSerif" "Twitter Color Emoji")
      '("Symbola" "Noto Color Emoji"
        "Liberation Mono"
        "Segoe UI Emoji" "FreeSerif" "Twitter Color Emoji")
      )
    ...
    (when (display-graphic-p)
      ...
      (dolist (face my/main-fonts)
        (let ((font-name (cdr face))
              (face-name (car face)))
          (set-face-attribute face-name nil :family font-name :height 120)))
      ...
      (set-fontset-font t 'cyrillic (font-spec :family "Noto Sans"))
...
#+begin_src emacs-lisp
  (setup eldoc
    (:option eldoc-documentation-strategy #'eldoc-documentation-compose
...
** Folding
Code folding with hideshow (hs-minor-mode).
#+begin_src emacs-lisp
  (setup hs-minor-mode
    ;; Enable in every programming mode
    (:hook-into prog-mode)
    ;; Fold comments along with code, and auto-open folds during isearch
    (:option hs-hide-comments-when-hiding-all t
             hs-isearch-open t
             hs-hide-initial-comment-block t)
    ;; Extra bindings on the C-c z prefix (kl-code-prefix-map).
    ;; hs-minor-mode lives in the `hideshow' library, so wrap in
    ;; :with-feature so the bindings wait for the right feature.
    (:with-feature hideshow
      (:with-map kl-code-prefix-map
        (:bind "t" hs-toggle-hiding)
        (:bind "h" hs-hide-block)
        (:bind "s" hs-show-block)
        (:bind "l" hs-hide-level)
        (:bind "H" hs-hide-all)
        (:bind "S" hs-show-all)))
    ;; Show the number of hidden lines in the fold label
    (defun kl/hs-set-up-overlay (ov)
      (when (eq 'code (overlay-get ov 'hs))
        (overlay-put ov 'display
                     (format "… %d lines"
                             (1+ (count-lines (overlay-start ov)
                                              (overlay-end ov)))))))
    (:option hs-set-up-overlay 'kl/hs-set-up-overlay))
#+end_src
...
#+begin_src emacs-lisp
  (setup elisp-mode
    (:hook flycheck-mode outline-minor-mode indent-bars-mode)
...
#+begin_src emacs-lisp
  (setup telega
    (:package telega)
    (:option telega-avatar-workaround-rsvg-bugs t)
   )
#+end_src
...
#+begin_src emacs-lisp
  (setup kkp
    (:package kkp)
    (:with-function tty-setup
      (:hook global-kkp-mode)
      )
    )
#+end_src
* Thanks
